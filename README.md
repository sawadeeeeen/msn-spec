...
> 「`sawadeeeeen/msn-spec` のインストールが完了したので、このリポジトリのルートにある `AGENTS.md` に書かれている自動ルーティングの指示を、このプロジェクトの `.agents/AGENTS.md` に既存の記述を壊さないように追記してください」

##### 🛠️ 設定方法 B: コマンドで末尾に追記する
すでに既存の `.agents/AGENTS.md` が存在し、手動でマージする場合は、以下のコマンドを実行して末尾に追記（アペンド）してください。

```bash
# 既存の定義の末尾に追記する（上書きを防止）
cat AGENTS.md >> .agents/AGENTS.md
```
※ `.agents/AGENTS.md` がまだ存在しない場合は、新規作成した上で追記してください。
