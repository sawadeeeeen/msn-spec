...
- **[PRE-SPEC-01] パッケージインストール**: npx skills 規格およびカスタムスキルをサポートするAIエージェント環境の設定ディレクトリに、3つのスキル（`messi`, `suarez`, `neymar`）が配置されていること。
- **[PRE-SPEC-02] 実行コンテキスト**: コマンドの実行は、対象プロジェクトのルートディレクトリであること（ファイル参照を正しく通すため）。

### 3.2. 事後条件 (Post-conditions)
- **[POST-SPEC-01] ファイル更新の永続化**: 各スキルの実行完了時、作成または修正されたファイル（`[機能名]-SSoT.md`、`.agents/gdd-reports/[機能名]-gdd-validator-report.md`）は必ずローカルディスクに保存（永続化）され、別セッションに引き継げる状態になること。
- **[POST-SPEC-02] レポートの非公開化**: 監査テストレポートは、`.agents/` などの隠しフォルダに保存されるため、Gitにコミットおよび共有されないこと（`.gitignore` で除外されていること）。

### 3.3. 不変条件 (Invariants / パッケージが常に維持すべきルール)
- **SSoTの純度 (What vs How)**: `[機能名]-SSoT.md` には「何をするか（What）」のみを定義し、「どう実装するか（How）」を含めてはならない。
- **Guarantees（表明）への一意のID付与の義務化 (ISO 29148準拠)**: 仕様書内の事前条件、事後条件、不変条件には、追跡可能性を保証するための一意の識別ID（例: `[PRE-AUTH-01]`, `[POST-AUTH-01]`, `[INV-AUTH-01]`）を必ず付与すること。
- **意思決定テーブルの強制 (プレイン・ランゲージ準拠)**: 条件分岐が3つ以上重なる複雑なロジックを仕様書に記載する際は、曖昧な文章表現を避け、必ず決定テーブル（マトリクス表）を用いて記述すること。
...
- **LSP置換原則の監査 (DbC準拠)**: 仕様の差分更新時、既存の事前条件の厳格化（強化）や、事後条件・不変条件の緩和（弱化）を検出した場合は、不適合（`FAIL`）と判定すること。
